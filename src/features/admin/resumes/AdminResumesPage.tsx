...
const STATUS_OPTIONS: { label: string; value: ResumeStatus; color: string }[] = [
  { label: 'Pending',   value: 'PENDING',   color: 'orange' },
  { label: 'Reviewing', value: 'REVIEWING', color: 'blue'   },
...
const AdminResumesPage: React.FC = () => {
  ...
  const load = useCallback(() => {
    setLoading(true)
    resumeService.getAll({ page, size: 10, email: keyword, status: statusFilter })
  ...
  const columns = [
    { title: 'Candidate', key: 'user', render: (_: any, r: Resume) => (
      <div><div style={{ fontWeight: 600 }}>{r.user?.name}</div><div style={{ fontSize: 12, color: 'var(--tx3)' }}>{r.email}</div></div>
    )},
    ...
    { title: 'Status', key: 'status', render: (_: any, r: Resume) => (
      <Select value={r.status} size="small" style={{ width: 130 }}
    ...
    { title: '', key: 'actions', render: (_: any, r: Resume) => (
  ...
  return (
    <div className={styles.root}>
      <div className={styles.pageHead}>
        <div><h1 className={styles.title}>Resumes</h1><p className={styles.sub}>{total.toLocaleString()} total applications</p></div>
        <Space>
          <Input prefix={<SearchOutlined />} placeholder="Search email..." value={keyword}
